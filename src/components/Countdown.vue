...
function drawSemiArc() {
  ...
  ctx.stroke()
  ctx.closePath()
  ctx.font = "500 24px SF Pro Display"
  let value = `${props.more ? props.more : ''}`
  let mx = ctx.measureText(value)
  ctx.fillText(value, state.width / 2 - mx.width / 2, state.height / 2 + 24 / 3)
