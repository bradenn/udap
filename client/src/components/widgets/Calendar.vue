...
<template>
  <div v-if="!state.loading" class="d-flex flex-column gap-1"
    ...
    <div class="element hour-grid">
      <div v-for="cal in state.calendar" v-if="state.calendar.length > 0"
           :key="cal.description"
           class="subplot  p-1 px-2 d-flex justify-content-between cal-event">

        <div>
          <div
              class="label-c1 label-r label-w500 label-o5 lh-sm overflow-ellipse">
            {{ cal.summary }}
          </div>
        ...
        <div class="d-flex flex-column justify-content-center align-items-end">
          ...
          <div class="label-c2 label-o4">{{ getTime(cal.start) }} -
            {{ getTime(cal.end) }}
          </div>
        </div>
      </div>
      <div v-else
           class="subplot subplot-inline p-1 px-2 d-flex justify-content-between cal-event">
        <div>
