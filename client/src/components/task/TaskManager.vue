...
<template>
  <div class="d-flex gap-1 w-100" style="height:100%">
    ...
    <div class="d-flex flex-column gap-1 w-100">
      <div
          class="element d-flex flex-row justify-content-around gap-1 p-1">
        <div v-for="task in state.tasks" :id="task.title" class=" w-100 ">
          <div
              :class="`${task.title === state.current.title?'':'subplot-inline'}`"
              class="subplot d-flex justify-content-center gap-1 "
              style="height: 2rem; border-radius: 0.25rem !important;"
              @click="selectTask(task)">
            <div
                class="px-1 lh-1 d-flex flex-column align-items-center gap-1 ">
              <div
                  :class="`${task.title === state.current.title?'text-accent':''}`"
                  class="label-c2 label-o4 label-w500 lh-1">{{
                  task.title
                }}
              </div>
              <div class="label-c3 label-o3 label-w500 lh-1"
                   style="max-width: 4rem; text-overflow:clip; text-wrap: none">
                {{
                  task.preview
                }}
              </div>
      ...
      <div class="element">
        <div v-if="state.loaded"
          ...
          <div class="label-md label-w700 label-o5 px-1 lh-1 mb-2 mt-1">
            {{ state.current.title }}
          </div>
          <div class="label-c2 label-w400 label-o3 px-2 lh-1 mb-2 mt-1">
        ...
        <FixedScroll class="" style="max-height: 12rem; overflow-y: scroll;">
          <div>
            <div v-if="state.current.type === TaskType.String">
              <div
                  class="d-flex mb-1 align-items-center justify-content-center">
              ...
              <div class="selection-grid px-1 pt-1">
                <div v-for="option in state.current.options"
                  ...
                  <div class="d-flex flex-column p-1">
                    ...
                    <div class="lh-1 label-c3 label-o3">{{
                        option.description
                      }}
                    </div>
                  </div>
        ...
        <div class="d-flex justify-content-between align-items-center mt-1">
          <div class="label-c2 label-w400 label-o3 px-2 lh-1 ">
            {{ state.current.description }}
          </div>
          <div class="subplot  d-flex justify-content-center"
               style="height:1.5rem; min-width:3.5rem;"
               @click="() => nextTask()">
            <div
