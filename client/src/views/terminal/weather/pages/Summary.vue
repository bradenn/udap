...
<template>
  <div class="d-flex flex-column gap-1">
    <div v-if="!state.loading" class="element p-2 pt-1">
      <div class=" d-flex flex-row align-items-center">
        <div class="flex-shrink-1 "
             style="min-width: 9rem; padding-left: 0.25rem">
          <h2 class="lh-md">
            {{ roundDecimal(state.latest.current_weather?.temperature, 0) }}°
            F</h2>
          <div class="label-c1 label-r label-w400 label-o4 lh-1">{{
              getWeatherState(state.latest.current_weather?.weathercode)
            }}
          </div>
          <div class="label-c1 label-r label-o3 label-w400 ">High
            {{ Math.round(state.ranges.temp.max) }}° •
            Low
            {{ Math.round(state.ranges.temp.min) }}°
          </div>
        </div>
        <div v-if="state.latest.hourly"
          ...
          <div
            ...
            <div v-if="state.latest.hourly?.temperature_2m[hour]"
              ...
              <div
                  v-if="state.latest.hourly.precipitation[new Date().getHours()+hour] <= 0.0001"
    ...
    <div v-if="!state.loading" class="d-flex gap-1 flex-row">
      <PaneList :alt='timeSince(state.lastUpdate)'
                style="width: 13rem !important; height: 14rem !important;"
                title="Today">
      ...
      <div class=" d-flex flex-column gap-1 w-100">
        <div class="element d-flex flex-column p-2 pt-1">
          <div class="d-flex justify-content-between lh-1">
            ...
            <div
                v-if="state.latest.hourly.precipitation[new Date().getHours()] >= 0.0001"
