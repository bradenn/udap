...
<template>
    <div class="page-grid">
        <PaneMenu :previous="false" alt="This terminal" title="Terminal">
            <PaneListItem :active="false" :subtext="`${formatByteSize(state.terminal.maxRSS)}`"
                          title="MaxRSS"></PaneListItem>
            <PaneListItem :active="false" :subtext="`${state.terminal.objects}`" title="Objects"></PaneListItem>
            <PaneListItem :active="false" :subtext="`${state.terminal.lastTarget}`" title="Last Target"></PaneListItem>
            <PaneListItem :active="false" :subtext="`${state.avgInterval}`" title="Burst Interval"></PaneListItem>
            <PaneListItem :active="false" :subtext="`${state.avgInterval}`"
                          title="Last Update"></PaneListItem>
        </PaneMenu>
        <div v-if="state.loaded" class="element">
            <div v-for="item in state.sortedQueue" :key="item.id" class="d-flex justify-content-between">
                <div>
        ...
        <div>
            ...
            <LiveChart v-if="state.loaded" :marker="122"
                       :scale="2" :values="state.maxRSSHistory.map(h => Math.round(h/1024*100)/100)"
