...
import type {Endpoint, Task, TaskOption} from "@/types";
...
function finish(tasks: Task[]) {
    ...
    endpointService.createEndpoint({
        ...
        notifications.success("Endpoint", `Created endpoint '${name.value}' successfully.`)
        goBack()
    }).catch(err => {
        notifications.fail("Endpoint", `Failed to create endpoint '${name.value}'.`, err)
    })
            ...
            <div class="d-flex justify-content-center h-100 w-100"
                ...
                <div class="generic-grid h-100" @click.stop>
                    <div>
                        ...
                        <TaskManager :on-complete="finish" :tasks="state.tasks" :title="`Endpoint`"
                                     class="h-100"></TaskManager>
                    </div>
                </div>
            </div>
