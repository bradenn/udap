...
].map(c => `${Math.round(c[0] * 255)}, ${Math.round(c[1] * 255)}, ${Math.round(c[2] * 255)}`)

function viridisRGB(value: number): string {
  return `rgb(${getViridis(value)})`
}
...
function groupByDay(t: Trace[]): Trace[] {

  let days: Trace[][] = []

  let date = 0

  for (let j = 0; j < t[0].time.length; j++) {
    let sum = 0
    let cnt = 0
    for (let i = 0; i < t.length; i++) {
      let trace = t[i]
      if (trace.time[j] != date) {
        date = trace.time[j]
        sum = 0
        cnt = 0
      }
...
function draw() {
  ...
  let days = transformed[0].data.length
